...
date: "最終更新: `r format(Sys.Date(), '%Y/%m/%d')`, 作成: 2021/4/12"
abstract: |
...
knitr::opts_chunk$set(
  fig.cap = if(!knitr::is_html_output() && !knitr::is_latex_output()) TRUE else NULL,
...
入門なので設定に関する細かい説明は省きます. まずは初期設定に従ってください. 4.0.5 以降の R (起動直後のメッセージや `R.version` で分かります.) と, 1.4 の RStudio 以降 (バージョンはヘルプや `RStudio.Version()` で分かります) がインストールされている前提です.
...
```{r eval=F, code = readLines("rmd-setup.R", encoding = "utf-8")}
```
...
Windows 10 をお使いの場合, 保存先のフォルダパスや .Rmd ファイルの名前に**日本語を使わないでください**. 現時点ではおそらくなんらかの不具合が起こる可能性が高いです. デフォルトでは OneDrive の「ドキュメント」フォルダになることが多いため注意してください.
...
```{r table-example2, tab.cap="kableExtra でスタイルを設定した例", tab.id = "table-example2", tab.cap.style = "Table Caption", message=F}
require(kableExtra)
mtcars[1:8, 1:8] %>%
