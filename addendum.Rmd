...
```{r make-bib, include=F, eval=F}
knitr::write_bib(
  c(.packages(),
  c("rmarkdown", "bookdown", "tinytex", "ggplot2", "citr",
    "bookdownplus", "govdown", "rticles",
    "blogdown", "rmdformats", "pagedown", "flexdashboard",
    "distill", "tufte", "gm", "xaringan", "ymlthis", "officedown",
    "officer", "redoc"
    )), file = 'packages.bib')
```
...
```{r format-tab, echo=F, tab.cap="フォーマット", tab.id = "format-tab", tab.cap.style = "Table Caption"}
...
), booktabs = T, col.names = c("ファイル形式", "フォーマット関数名"), caption = "主な出力フォーマット")
```
...
```{r, ggplot-example, fig.cap ="ggplot2 によるグラフ", fig.id = "ggplot-example", fig.cap.style = "Image Caption", warning=F}
data(mtcars)
ggplot(mtcars, aes(x = mpg, y = disp, color = cyl)) +
  geom_point() + labs(x = "マイル/ガロン", y = "排気量")
```
...
```{r tokyor-logo, fig.cap="Tokyo.R のロゴ", fig.id = "tokyor-logo", fig.cap.style = "Image Caption"}
...
```{r plot-example, fig.cap = "標準正規分布", warning=F, fig.id = "plot-example", fig.cap.style = "Image Caption"}
...
```{r table-example, tab.cap="表の例", tab.id = "table-example", tab.cap.style = "Table Caption"}
data(mtcars)
...
```{r table-example2, tab.cap="kableExtra でスタイルを設定した例", tab.id = "table-example2", tab.cap.style = "Table Caption"}
require(kableExtra)
mtcars[1:8, 1:8] %>%
...
```{r ymlthis, fig.cap="(ref:cap-ymlthis)", fig.id = "ymlthis", fig.cap.style = "Image Caption"}
...
```{r vme-citation, fig.cap="Visual Markdown Editor による引用リスト", fig.id = "vme-citation", fig.cap.style = "Image Caption"}
